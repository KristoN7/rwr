...
func _ready() -> void:
	...
	# Łączymy sygnał wciśnięcia przycisku z naszą funkcją
	button_pressed.connect(_on_button_pressed)

func _process(_delta: float) -> void:
	# Pokazywanie markera na ziemi
	if ray.is_colliding():
...
func _on_button_pressed(button_name: String) -> void:
	# Reakcja na główny spust lub przycisk A/X
	if button_name == "trigger" or button_name == "primary_click":
		teleport_now()
...
func teleport_now() -> void:
	...
	# Wymóg na ocenę 5.0: offset głowy zostaje uwzględniony (rzutowany poziomo)
	cam_offset.y = 0.0
